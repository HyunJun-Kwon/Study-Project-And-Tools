...
function idCheck() {
	...
	formData.append('id', document.frm.id.value);
	...
	fetch(url, {
		...
		.then(data => {
			window.open("idCheck?result=" + data.result + "&id=" + data.id, "_blank", "toolbar=no, menubar=no, scrollbars=yes, resizable=no, width=450, height=200");
			console.log(data);
		})
		.catch(error => {
...
function idok(id) {
	...
	opener.frm.reid.value = id;  //중복 검사를 한 아이디인지 아닌지를 저장
	self.close();
}
